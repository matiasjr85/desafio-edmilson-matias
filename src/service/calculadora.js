...
import { verificarExtra } from "./verificar-extra.js";
import { descontosTaxas } from "./descontos-taxas.js";
...
function Calculadora(formaDePagamento, pedidos) {
  ...
  for (let i = 0; i < comanda.length; i++) {
    if (comanda[i].vinculo.length > 0) {
      if (!verificarExtra(comanda, comanda[i].vinculo)) {
  ...
  return descontosTaxas(conta, formaDePagamento);
}
